...
def wordScoreLoop(S, scorelist):
    ...
    total=0
    for x in S:
        total+=letterScoreLoop(x, scrabbleScores)
    return total
    
print(wordScoreLoop('dog', scrabbleScores))
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
' Exercise 5
...
def wordsWithScoreLoop(dct, scores):
    ...
    n=len(dct)
    for i in range(n):
        score+=[[dct[i],wordScore(dct[i], scores)]]
        i+=1
    return score

print(wordsWithScoreLoop(['python', 'is', 'awesome'],scrabbleScores))